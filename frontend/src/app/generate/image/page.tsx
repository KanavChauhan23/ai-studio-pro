...
export default function ImagePage() {
  ...
  const generate = () => {
    ...
    setImages(Array.from({ length: n }, (_,i) => ({
      url: `https://image.pollinations.ai/prompt/${encodeURIComponent(enhanced)}?width=${w}&height=${h}&seed=${Math.floor(Math.random()*99999)+i*1337}&nologo=true`,
          ...
          <div className="img-grid" style={{ gridTemplateColumns: images.length === 1 ? '1fr' : '1fr 1fr' }}>
            {images.map((img, i) => (
              <div key={i} className="img-c">
                {!img.loaded && <div className="img-skel"><span className="spin spin-v" style={{ width:32,height:32,borderWidth:3 }} /></div>}
                {!img.error && (
                  <img src={img.url} alt={`Gen ${i+1}`}
                    style={{ width:'100%', display: img.loaded ? 'block' : 'none', borderRadius:'16px 16px 0 0' }}
                    onLoad={() => onLoad(i)} onError={() => onError(i)} />
                )}
                {img.error && (
