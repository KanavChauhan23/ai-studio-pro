  ...
  return (
    <aside className="sidebar" style={{ width: W }}>
      <div className="sb-head">
        {!col && (
          <Link href="/dashboard" style={{ display:'flex', alignItems:'center', gap:9, textDecoration:'none', flex:1, minWidth:0 }}>
            <div className="sb-mark">⚡</div>
            <span className="sb-name">AI Studio Pro</span>
          </Link>
        )}
        {col && <div className="sb-mark" style={{ margin:'0 auto' }}>⚡</div>}
        {!col && <button className="btn-ic" onClick={() => setCol(true)} title="Collapse" style={{ fontSize:12, flexShrink:0, width:26, height:26 }}>‹</button>}
        {col  && <button className="btn-ic" onClick={() => setCol(false)} title="Expand" style={{ fontSize:12, position:'absolute', right:4, top:18, width:26, height:26 }}>›</button>}
      </div>

      <div className="sb-body">
        {NAV.map(n => (
          <Link key={n.href} href={n.href} className={`sbn ${pathname===n.href?'on':''}`} title={col?n.lb:undefined}>
            <span className="sbn-ic">{n.ic}</span><span className="sbn-lbl">{n.lb}</span>
          </Link>
        ))}
        <div className="sep" style={{ margin:'8px 0' }} />
        {!col && <div className="sb-lbl">Tools</div>}
        {TOOLS.map(t => (
          <Link key={t.href} href={t.href} className={`sbn ${pathname===t.href?'on':''}`} title={col?t.lb:undefined}>
            <span className="sbn-ic">{t.ic}</span><span className="sbn-lbl">{t.lb}</span>
          </Link>
        ))}
      </div>
      ...
      <div className="sb-foot">
        <button className="sb-user" onClick={() => { logout(); router.push('/login'); }}>
          ...
          {!col && <div><div className="sb-uname">{user?.username || 'User'}</div><div className="sb-uplan">Free · Sign out</div></div>}
        </button>
      </div>
    </aside>
  );
